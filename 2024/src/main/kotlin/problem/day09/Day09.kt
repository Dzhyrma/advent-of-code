...
fun solveDay09Part1(input: List<String>): Long {
    ...
    var currentMemoryBlockIndex = 0L
    ...
    memoryHash.forEachIndexed { index, length ->
        when {
            ...
            // file
            index % 2 == 0 -> {
                checksum += fileChecksum(index shr 1, length, currentMemoryBlockIndex)
                currentMemoryBlockIndex += length
            }
            // empty memory space
            else -> {
                ...
                while (emptyMemoryLength > 0 && memoryLastFileIndex >= index) {
                    ...
                    checksum += fileChecksum(memoryLastFileIndex shr 1, fileChunkLength, currentMemoryBlockIndex)
                    memoryHash[memoryLastFileIndex] -= fileChunkLength
                    emptyMemoryLength -= fileChunkLength
                    currentMemoryBlockIndex += fileChunkLength
...
fun solveDay09Part2(input: List<String>): Long {
    ...
    return files.mapIndexed { fileId, (fileBlockIndex, fileLength) ->
        fileChecksum(fileId, fileLength, fileBlockIndex.toLong())
    }.sum()
}
...
private fun fileChecksum(fileId: Int, fileLength: Int, memoryBlockIndex: Long) =
